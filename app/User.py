...
This file contains the class User which inherits from the class BaseModel from the Pydantic library.
...
The User class defines attributes username, hash_password, is_admin, locations and weather_reports.
The class methods allow the user to interact with their userspace with output events such as
add_surf_location, remove_surf_location, display_userspace, get_weather and ask_on_surf_the_best_spot

The userspace is an abstract idea which concretely is the combination of locations of type Location
and weather_reports of type WeatherReport as dictionaries for a specific user.

Finally, the static private method _fill_weather_report post a weather request to the windy API:
...
A timestamp filters the api response to keep only the values for the date and time the user asked a weather report for.
The weather keys are renamed and the values are passed in the weather_report dictionary of the user.
note:
