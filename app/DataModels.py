...
The class CoordinatesGPS ensures that the attributes longitude and latitude will be
constraint to be float types restricted to the values between -180, 180 and -90, 90, respectively.
...
The last class WeatherReport ensures that each attribute: waves_height, waves_direction, wind_u, wind_v and timestamp
are passed information as defined in the model.
...
The wind_speed method returns the wind speed calculated by finding the distance between attribute wind_u and wind_v.
This speed is given in knots to the user using the method wind_speed_knots.
Lastly, wind_direction returns the wind direction in degrees from the polar angle of the wind_speed vector,
